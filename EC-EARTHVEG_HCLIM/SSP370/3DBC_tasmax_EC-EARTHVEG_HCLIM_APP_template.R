...
for (p in 1:length(idy))
{
  ...
  refsimfile <- paste("/lustre/storeC-ext/users/kin2100/MET/3DBC/application/CMIP6/work/tasmax/Cur/ecearthveg-r1i1p1f1-hclim_hist_eqm-sn2018v2005_rawbc_norway_1km_tasmax_daily_",RefYear,".nc4",sep="")
  if (RefYear > 2020)
    refsimfile <- paste("/lustre/storeC-ext/users/kin2100/MET/3DBC/application/CMIP6/work/tasmax/Fut/ecearthveg-r1i1p1f1-hclim_ssp370_eqm-sn2018v2005_rawbc_norway_1km_tasmax_daily_",RefYear,".nc4",sep="")
  ...
  nc <- nc_open(paste("/lustre/storeC-ext/users/kin2100/MET/3DBC/application/CMIP6/work/tasmax/Fut/ecearthveg-r1i1p1f1-hclim_ssp370_eqm-sn2018v2005_rawbc_norway_1km_tasmax_daily_",YEAR,".nc4",sep=""))
  FutA <- ncvar_get(nc,"tasmax",start = c(1,idy[p],1), count=c(-1,szy[p],-1))
  nc_close(nc)
  
  # Mean temperature (as lower limit)
  nc <- nc_open(paste("/lustre/storeC-ext/users/kin2100/MET/3DBC/application/CMIP6/work/tas/FutC/ecearthveg-r1i1p1f1-hclim_ssp370_3dbc-eqm-sn2018v2005_rawbc_norway_1km_tas_daily_",YEAR,".nc4",sep=""))
  ...
  for (i in 1:NofPoints)  
  {
    ...
    ##Use mean temperature as lower limit (analog seNorge)
    RankFC <- nodp1 -  rank(GaussFC,ties.method = "first")
    FutC_uc <- sort(Fut,decreasing = TRUE)[RankFC]
    FutCA[x,y,] <- pmax(FutC_uc,Fut_tn)
  }
  #That's all :-)
  
  #Write to NetCDF
  nc <- nc_open(paste("/lustre/storeC-ext/users/kin2100/MET/3DBC/application/CMIP6/work/tasmax/FutC/ecearthveg-r1i1p1f1-hclim_ssp370_3dbc-eqm-sn2018v2005_rawbc_norway_1km_tasmax_daily_",YEAR,".nc4",sep=""),write=TRUE)
...
print("==========================================")
print("Recompression")
ifile <- paste0("/lustre/storeC-ext/users/kin2100/MET/3DBC/application/CMIP6/work/tasmax/FutC/ecearthveg-r1i1p1f1-hclim_ssp370_3dbc-eqm-sn2018v2005_rawbc_norway_1km_tasmax_daily_",YEAR,".nc4")
