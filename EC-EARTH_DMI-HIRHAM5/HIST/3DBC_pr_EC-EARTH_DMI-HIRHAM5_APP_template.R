...
ObsA <- ncvar_get(nc,"rr")/24/3600 #convert to mm/seconds
nc_close(nc)

nc <- nc_open(paste("/lustre/storeB/users/andreasd/KiN_2023_data/3DBC/pr/Cur/ecearth-r3i1p1-hirham_hist_eqm-sn2018v2005_rawbc_norway_1km_pr_daily_",RefYear,".nc4",sep=""))
CurA <- ncvar_get(nc,"pr")
nc_close(nc)
...
nc <- nc_open(paste("/lustre/storeB/users/andreasd/KiN_2023_data/3DBC/pr/Cur/ecearth-r3i1p1-hirham_hist_eqm-sn2018v2005_rawbc_norway_1km_pr_daily_",YEAR,".nc4",sep=""))
FutA <- ncvar_get(nc,"pr")
nc_close(nc)
...
nc <- nc_open(paste("/lustre/storeB/users/andreasd/KiN_2023_data/3DBC/pr/CurC/app/ecearth-r3i1p1-hirham_hist_3dbc-eqm-sn2018v2005_rawbc_norway_1km_pr_daily_",YEAR,".nc4",sep=""),write=TRUE)
ncvar_put(nc,"pr",FutCA)
nc_close(nc)
