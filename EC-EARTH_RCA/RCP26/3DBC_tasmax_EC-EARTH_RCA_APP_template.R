...
for (p in 1:length(idy))
{
  ...
  refsimfile <- paste("/lustre/storeB/users/andreasd/KiN_2023_data/3DBC/tasmax/Cur/ecearth-r12i1p1-rca_hist_eqm-sn2018v2005_rawbc_norway_1km_tasmax_daily_",RefYear,".nc4",sep="")
  if (RefYear > 2014)
    refsimfile <- paste("/lustre/storeB/users/andreasd/KiN_2023_data/3DBC/tasmax/Fut/ecearth-r12i1p1-rca_rcp26_eqm-sn2018v2005_rawbc_norway_1km_tasmax_daily_",RefYear,".nc4",sep="")
  ...
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/KiN_2023_data/3DBC/tasmax/Fut/ecearth-r12i1p1-rca_rcp26_eqm-sn2018v2005_rawbc_norway_1km_tasmax_daily_",YEAR,".nc4",sep=""))
  FutA <- ncvar_get(nc,"tasmax",start = c(1,idy[p],1), count=c(-1,szy[p],-1))
  nc_close(nc)
  
  # Mean temperature (as lower limit)
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/KiN_2023_data/3DBC/tas/FutC/ecearth-r12i1p1-rca_rcp26_3dbc-eqm-sn2018v2005_rawbc_norway_1km_tas_daily_",YEAR,".nc4",sep=""))
  ...
  #Write to NetCDF
  nc <- nc_open(paste("/lustre/storeB/users/andreasd/KiN_2023_data/3DBC/tasmax/FutC/ecearth-r12i1p1-rca_rcp26_3dbc-eqm-sn2018v2005_rawbc_norway_1km_tasmax_daily_",YEAR,".nc4",sep=""),write=TRUE)
